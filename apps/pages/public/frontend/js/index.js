//// Default vars ////
var API = window.EMULATORJS_API_BASE || '';
...
var loadvideo = debounce(function(active_item) {
  ...
  if (has_video) {
    ...
    var video_path = API + '/api/art/' + $('#i' + active_item.toString()).data('path') + '/videos/';
...
var loadart = debounce(function(active_item) {
  ...
  var path = API + '/api/art/' + $('#i' + active_item.toString()).data('path') + '/';
...
function loadlogos(logo_load_start, display_items, items_length, active_item) {
  for (var i = 0; i < display_items; i++) {
    ...
    if (has_logo) {
      var path = API + '/api/art/' + $('#i' + item_num.toString()).data('path') + '/logos/';
...
function launch(active_item) {
  ...
  if (type == 'menu') {
    ...
    var rom_path = API + '/api/roms/' + path + '/';
    var rom_extension = $('#i' + active_item.toString()).data('rom_extension');
    var bios = API + '/api/bios/' + path + '/' + $('#i' + active_item.toString()).data('bios');
    // Clear screen
    $('body').empty();
    // Add game window
...
async function loadjson(name, active_item) {
  if (name == 'preview') {
    var url = API + '/api/config/preview';
  } else {
    var url = API + '/api/config/' + name;
  }
