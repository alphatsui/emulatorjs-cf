//// Default vars ////
var API_BASE = window.EMULATORJS_API_BASE || '';
...
var loadvideo = debounce(function(active_item) {
  ...
  if (has_video) {
    ...
    var video_path = API_BASE + '/api/art/' + $('#i' + active_item.toString()).data('path') + '/videos/';
...
var loadart = debounce(function(active_item) {
  ...
  var consolePath = $('#i' + active_item.toString()).data('path');
  var back_src = API_BASE + '/api/art/' + consolePath + '/backgrounds/' + back_name + '.png'
  var corner_src = API_BASE + '/api/art/' + consolePath + '/corners/' + corner_name + '.png'
...
function loadlogos(logo_load_start, display_items, items_length, active_item) {
  for (var i = 0; i < display_items; i++) {
    ...
    if (has_logo) {
      var path = API_BASE + '/api/art/' + $('#i' + item_num.toString()).data('path') + '/logos/';
...
function launch(active_item) {
  ...
  if (type == 'menu') {
    ...
    var rom_path = API_BASE + '/api/roms/' + path + '/';
    var rom_extension = $('#i' + active_item.toString()).data('rom_extension');
    var bios = API_BASE + '/api/roms/' + path + '/bios/' + $('#i' + active_item.toString()).data('bios');
    // Clear screen
    $('body').empty();
    // Add game window
    ...
    if (bios !== API_BASE + '/api/roms/' + path + '/bios/') {
      EJS_biosUrl = bios;
    }
    EJS_player = '#game';
...
async function loadjson(name, active_item) {
  if (name == 'preview') {
    var url = API_BASE + '/api/config/preview';
  } else {
    var url = API_BASE + '/api/config/' + name;
  }
